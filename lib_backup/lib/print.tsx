...
function openPrintWindow(title: string, accentColor: string, bodyHtml: string) {
  ...
  doc.open();
  // @ts-ignore: doc.write is deprecated but required for print popup
  doc.write('<!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8">');
  // @ts-ignore: doc.write is deprecated but required for print popup
  doc.write(`<title>${title}</title>`);
  // @ts-ignore: doc.write is deprecated but required for print popup
  doc.write(`<style>:root { --accent: ${accentColor}; }${BASE_STYLE}</style>`);
  // @ts-ignore: doc.write is deprecated but required for print popup
  doc.write('</head><body>');
  // @ts-ignore: doc.write is deprecated but required for print popup
  doc.write(bodyHtml);
...
export function printBL(bl: BonLivraison) {
  ...
  const lignesRows = bl.lignes.map((l, i) => {
    const umHtml = l.quantiteUM && l.um
      ? `<span style="color:#20232a" class="um-badge">${l.quantiteUM} ${l.um}</span>`
      : ""
    return `<tr>
      <td>${i + 1}</td>
  ...
  const statutBadge = {
    livre:           '<span style="color:#20232a" class="badge badge-green">Livré</span>',
    premier_passage: '<span style="color:#20232a" class="badge badge-blue">1er Passage</span>',
    deuxieme_passage:'<span style="color:#20232a" class="badge badge-orange">2e Passage</span>',
    retour:          '<span style="color:#20232a" class="badge badge-red">Retour</span>',
  }[bl.statutLivraison] ?? ""
...
<div class="info-grid">
  <div class="info-box">
    <span style="color:#20232a" class="label">Client</span>
    <div class="value">${bl.clientNom}</div>
    <div class="sub">${bl.secteur || ""}${bl.zone ? ` — ${bl.zone}` : ""}</div>
  </div>
  <div class="info-box">
    <span style="color:#20232a" class="label">Livreur</span>
...
<div className="totaux-wrap">
  <div className="totaux-box">
    ...
    ${fraisImpression > 0 ? `<div className="total-row" style="color:#6b7280"><span style="color:#20232a">Frais d&apos;impression (${nbFeuilles} feuille${nbFeuilles > 1 ? "s" : ""} × ${fmt(fraisImpressionParFeuille)} DH)</span><span style="color:#20232a">${fmt(fraisImpression)} DH</span></div>` : ""}
    ${fraisService > 0 ? `<div className="total-row" style="color:#6b7280"><span style="color:#20232a">Frais de service (${totalNbCaisses} caisse${totalNbCaisses > 1 ? "s" : ""} × ${fmt(fraisServiceParCaisse)} DH)</span><span style="color:#20232a">${fmt(fraisService)} DH</span></div>` : ""}
    ${tva > 0 ? `<div className="total-row"><span style="color:#20232a">TVA (${tva}%)</span><span style="color:#20232a">${fmt(totalHTAvecCaisses * tva / 100)} DH</span></div>` : ""}
    <div className="total-row grand"><span style="color:#20232a">TOTAL TTC</span><span style="color:#20232a">${fmt(totalHTAvecCaisses + (tva > 0 ? totalHTAvecCaisses * tva / 100 : 0))} DH</span></div>
  </div>
</div>
...
<div class="doc-footer">${typeof coName !== "undefined" ? coName : ""} · Document généré le ${new Date().toLocaleDateString("fr-MA")} · Ce document vaut preuve de livraison.</div>
`
...
<div class="info-grid">
  <div class="info-box">
    <span style="color:#20232a" class="label">Émetteur</span>
    <div class="value">${coName}</div>
    <div class="sub">${coAddr || ""}${coIce ? ` · ${coIce}` : ""}</div>
  </div>
  <div class="info-box">
    <span style="color:#20232a" class="label">Client (Destinataire)</span>
    <div class="value">${bl.clientNom}</div>
    <div class="sub">${bl.secteur || ""}${bl.zone ? ` — ${bl.zone}` : ""}</div>
  </div>
</div>
...
<table class="erp-table w-full">
  <thead>
    <tr>
      <th class="text-[#111827] font-bold bg-[#f6f9fb]" style="width:28px">#</th>
      <th class="text-[#111827] font-bold bg-[#f6f9fb]">Désignation</th>
      <th class="text-[#111827] font-bold bg-[#f6f9fb] r" style="width:80px">Qté</th>
      <th class="text-[#111827] font-bold bg-[#f6f9fb] r" style="width:90px">P.U. HT (DH)</th>
      <th class="text-[#111827] font-bold bg-[#f6f9fb] r" style="width:60px">TVA</th>
      <th class="text-[#111827] font-bold bg-[#f6f9fb] r" style="width:110px">Montant HT (DH)</th>
    </tr>
  </thead>
  <tbody>
    ${lignesRows}
...
export function printBonCommande(cmd: Commande) {
  ...
  const statutBadge = {
    en_attente: '<span style="color:#20232a" class="badge badge-orange">En attente</span>',
    en_attente_approbation: '<span style="color:#20232a" class="badge badge-orange">En attente approbation</span>',
    valide: '<span style="color:#20232a" class="badge badge-green">Validée</span>',
    refuse: '<span style="color:#20232a" class="badge badge-red">Refusée</span>',
    en_transit: '<span style="color:#20232a" class="badge badge-blue">En transit</span>',
    livre: '<span style="color:#20232a" class="badge badge-green">Livrée</span>',
    retour: '<span style="color:#20232a" class="badge badge-red">Retour</span>',
  }[cmd.statut] ?? `<span style="color:#20232a" class="badge">${cmd.statut}</span>`;

  const totalCommande = cmd.lignes.reduce((s: any, l: any) => s + l.quantite, 0);

  const approbateurHtml = cmd.approbateur
    ? `<div class="sub">Approuvé par : ${cmd.approbateur}</div>`
    : "";
...
<div class="info-grid">
  ...
  <div class="info-box">
    <span style="color:#20232a" class="label">Prévendeur / Commercial</span>
...
<table class="erp-table w-full">
  <thead>
    <tr>
      <th class="text-[#111827] font-bold bg-[#f6f9fb]" style="width:28px">#</th>
      <th class="text-[#111827] font-bold bg-[#f6f9fb]">Article</th>
      <th class="text-[#111827] font-bold bg-[#f6f9fb] r" style="width:90px">Quantité</th>
      <th class="text-[#111827] font-bold bg-[#f6f9fb] r" style="width:90px">P.V. (DH)</th>
      <th class="text-[#111827] font-bold bg-[#f6f9fb] r" style="width:100px">Total (DH)</th>
    </tr>
  </thead>
  <tbody>
...
<div class="totaux-wrap">
  <div class="totaux-box">
    <div class="total-row">
      <span style="color:#20232a">Total commandé</span>
      <span style="color:#20232a">${totalCommande.toFixed(1)} kg</span>
    </div>
    <div class="total-row grand">
      <span style="color:#20232a">TOTAL HT</span>
      <span style="color:#20232a">${fmt(totalHT)} DH</span>
    </div>
  </div>
</div>
...
<table className="erp-table w-full">
  <th className="text-[#111827] font-bold bg-[#f6f9fb]"ead><tr>
    ...
    <th className="text-[#111827] font-bold bg-[#f6f9fb]">Article / Désignation</th>
    <th className="text-[#111827] font-bold bg-[#f6f9fb]" className="r" style="width:90px">Qté</th>
    <th className="text-[#111827] font-bold bg-[#f6f9fb]" className="r" style="width:90px">Unité</th>
    <th className="text-[#111827] font-bold bg-[#f6f9fb]" className="r" style="width:100px">P.U. (DH)</th>
    <th className="text-[#111827] font-bold bg-[#f6f9fb]" className="r" style="width:110px">Total (DH)</th>
  </tr></thead>
  <tbody>
    <tr>
      <td>1</td>
      <td><strong>${po.articleNom}</strong></td>
      <td className="r">${po.quantite}</td>
      <td className="r">${po.articleUnite}</td>
      <td className="r">${fmt(po.prixUnitaire)}</td>
      <td className="r"><strong>${fmt(po.total)}</strong></td>
    </tr>
  </tbody>
</table>
