        ...
        {isDemo && (
          <div className="flex items-center gap-2 px-4 py-2 bg-amber-50 border-b border-amber-200 text-amber-800 text-xs shrink-0">
            ...
            <span>
              ...
              <span className="">حساب تجريبي — التعديلات محلية فقط</span>
            </span>
          </div>
        )}
  ...
  return (
    <aside className="flex flex-col h-full bg-white border-r border-slate-200">
      ...
      <div className={`flex items-center gap-3 px-4 py-4 border-b border-slate-200 ${sidebarCollapsed ? "justify-center px-2" : ""}`}>
        ...
        {!sidebarCollapsed && (
          <div className="min-w-0">
            <p className="font-semibold" className="font-black text-sm leading-tight truncate">
      ...
      <nav className="flex-1 overflow-y-auto py-2 space-y-0.5 px-2 thin-scroll">
        ...
        {searchQ && filteredGroups.length === 0 && (
          <div className="flex flex-col items-center gap-2 py-8 text-slate-800">
            ...
            <p className="font-semibold" className="text-xs">Aucun resultat</p>
          </div>
        )}
      </nav>
        ...
        <div className="p-5 flex flex-col gap-4 overflow-y-auto max-h-[80vh]">
          ...
          <div className="flex flex-col items-center gap-3">
            <div className="relative">
              ...
              {canUseCamera ? (
                <label className="absolute bottom-0 right-0 w-7 h-7 rounded-full bg-primary text-white flex items-center justify-center cursor-pointer shadow border-2 border-card hover: transition-opacity" title="Modifier la photo (super admin)">
                  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="text-center">
              <p className="font-semibold" className="text-lg font-bold text-foreground">{user.name}</p>
              <p className="font-semibold" className="text-sm text-muted-foreground">{user.email}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2.5">
            <div className="rounded-xl bg-muted/40 border border-border p-3">
              <p className="font-semibold" className="text-[9px] font-bold uppercase tracking-widest text-muted-foreground mb-1.5">Role</p>
              <span className={`text-xs font-bold px-2 py-1 rounded-full text-white inline-block ${ROLE_COLORS[user.role]}`}>
                {ROLE_LABELS[user.role]}
              </span>
            </div>
            <div className="rounded-xl bg-muted/40 border border-border p-3">
              <p className="font-semibold" className="text-[9px] font-bold uppercase tracking-widest text-muted-foreground mb-1.5">Accès</p>
              <p className="font-semibold" className="text-sm font-semibold text-foreground capitalize">{user.accessType ?? "standard"}</p>
            </div>
            {user.secteur && (
              <div className="col-span-2 rounded-xl bg-muted/40 border border-border p-3">
                <p className="font-semibold" className="text-[9px] font-bold uppercase tracking-widest text-muted-foreground mb-1">Secteur</p>
                <p className="font-semibold" className="text-sm font-semibold text-foreground">{user.secteur}</p>
              </div>
            )}
            <div className="col-span-2 rounded-xl bg-muted/40 border border-border p-3">
              <p className="font-semibold" className="text-[9px] font-bold uppercase tracking-widest text-muted-foreground mb-1">ID</p>
              <p className="font-semibold" className="text-xs font-mono text-muted-foreground">{user.id}</p>
            </div>
          </div>

          {/* Permissions */}
          <div>
            <p className="font-semibold" className="text-[9px] font-bold uppercase tracking-widest text-muted-foreground mb-2">Permissions</p>
          ...
          <button onClick={onClose}
            className="w-full py-2.5 rounded-xl bg-primary text-primary-foreground font-semibold text-sm hover: transition-opacity">
            Fermer
          </button>
        </div>
      </div>
